...
commands_allowed = ['json.loads', 'manager', 'DataUtils', 'getMethods', 'manager.create_random_name()',
                    'manager.create_random_ipv4()']  # This list must contains the commands we want to have controll in our executions
...
class DataUtils(object):
    ...
    def is_a_command(expected_field):
        if expected_field.startswith('assert'):
            return False
        elif re.compile('|'.join(commands_allowed), re.IGNORECASE).search(expected_field) is not None:
            return True
